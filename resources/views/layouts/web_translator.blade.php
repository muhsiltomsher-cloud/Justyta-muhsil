...
<body class="min-h-screen flex flex-col">
    <div class="flex min-h-screen bg-[#FDF8F4] text-[#1A1A1A] px-[50px] gradient-primary !pt-10">
        ...
        <main class="flex-1 p-6 pe-0 pt-0 h-full">
            <!-- Header -->
            @include('frontend.translator.common.header')
                    ...
                    success: function(response) {
                        ...
                        if (response.length > 0) {
                            response.forEach(service => {
                                suggestions.append(
                                    `<a href="/user/service-request/${service.slug}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">${service.title}</a>`
                                    );
                            });
                            suggestions.show();
                        } else {
                            suggestions.append(
                                `<div class="px-4 py-2 text-sm text-gray-500">{{ __('frontend.no_result_found') }}</div>`
                                );
                            suggestions.show();
                        }
                    }
