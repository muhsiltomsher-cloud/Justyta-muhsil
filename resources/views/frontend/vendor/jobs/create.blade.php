...
@section('content')
    <div class="bg-white rounded-lg p-6">
        <h2 class="text-xl font-medium text-gray-900">{{ __('frontend.add_job') }}</h2>
        <hr class="my-4 border-[#DFDFDF]">

        <form class="grid grid-cols-1 md:grid-cols-4 gap-4" autocomplete="off" action="{{ route('jobs.store') }}"
            ...
            <div>
                ...
                <select name="emirate" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    @foreach (\App\Models\Emirate::with('translations')->get() as $emirate)
                        <option value="{{ $emirate->id }}" {{ old('emirate') == $emirate->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="type"
                    ...
                    <option value="">{{ __('frontend.choose_option') }}</option>
                    <option value="full_time" {{ old('type') == 'full_time' ? 'selected' : '' }}>{{ __('frontend.full_time') }}</option>
                    <option value="part_time" {{ old('type') == 'part_time' ? 'selected' : '' }}>{{ __('frontend.part_time') }}</option>
                </select>
                @error('type')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
                        ...
                        <div x-show="activeTab === '{{ $lang->code }}'" x-cloak class="space-y-5">
                            ...
                            <div>
                                ...
                                <textarea name="translations[{{ $lang->code }}][description]" @if ($lang->rtl) dir="rtl" @endif
                                    ...
                                    rows="6">{{ old('translations.' . $lang->code . '.description', '') }}</textarea>
