...
<div class="bg-white rounded-2xl  p-8 pb-12">
    ...
    <div class="mx-auto mt-10">
        ...
        <!-- Step Content -->
        <form id="lawyerForm" action="{{ route('vendor.update.lawyers', $lawyer->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <!-- Step 1 -->
            <div class="step-content" data-step="0">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    @foreach ($languages as $lang)
                        @php
                            $trans = $lawyer->translations->firstWhere('lang', $lang->code);
                        @endphp
                        <div>
                            ...
                            <input type="text" @if ($lang->rtl == 1) dir="rtl" @endif name="translations[{{ $lang->code }}][name]"
                                class="bg-[#F9F9F9] border border-gray-300 text-gray-900 text-sm rounded-[10px] focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5" value="{{ old('translations.' . $lang->code . '.name', $trans->full_name ?? '') }}" placeholder="{{ __('frontend.enter') }}">
                    ...
                    <div class="">
                        ...
                        <input type="email" name="email" id="email" placeholder="{{ __('frontend.enter') }}"
                            ...
                            value="{{ old('email', $lawyer->email) }}" readonly/>
                        @error('email')
                    ...
                    <div>
                        ...
                        <select name="gender" class="bg-[#F9F9F9] border border-gray-300 text-gray-900 text-sm rounded-[10px] focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5">
                            <option value="male" {{ old('gender', $lawyer->gender) == "male" ? 'selected' : '' }}>{{ __('frontend.male') }}</option>
                            <option value="female" {{ old('gender', $lawyer->gender) == "female" ? 'selected' : '' }}>{{ __('frontend.female') }}</option>
                        </select>
                        @error('gender')
                            <div class="text-sm text-red-500">{{ $message }}</div>
                        @enderror
                        ...
                        <select name="country" class="select2 bg-[#F9F9F9] border border-gray-300 text-gray-900 text-sm rounded-[10px] focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5">
                            ...
                            @foreach (\App\Models\Country::get() as $country)
                                <option value="{{ $country->id }}"
                                    {{ old('country', $lawyer->nationality) == $country->id ? 'selected' : '' }}>
                        ...
                        <select name="emirate_id" class="bg-[#F9F9F9] border border-gray-300 text-gray-900 text-sm rounded-[10px] focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5">
                            ...
                            @foreach (\App\Models\Emirate::with('translations')->get() as $emirate)
                                <option value="{{ $emirate->id }}"
                                    {{ old('emirate_id', $lawyer->emirate_id) == $emirate->id ? 'selected' : '' }}>
                    ...
                    <div class="">
                        ...
                        <select name="experience" class="bg-[#F9F9F9] border border-gray-300 text-gray-900 text-sm rounded-[10px] focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5">
                            ...
                            @foreach($dropdowns['years_experience']->options as $option)
                                <option value="{{ $option->id }}" {{ old('experience', $lawyer->years_of_experience) == $option->id ? 'selected' : '' }}>
                    ...
                    <div class="">
                        ...
                        <select name="specialities[]" id="select-tag" class="form-control select2 ip-gray radius-xs b-light px-15" multiple>
                            ...
                            @foreach($dropdowns['specialities']->options as $option)
                                <option value="{{ $option->id }}" {{ in_array($option->id, old('specialities', $specialityIds ?? [])) ? 'selected' : '' }}>
                                    {{ $option->getTranslation('name', app()->getLocale()) ?? '-' }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="">
                        ...
                        <div id="logoPreview" class="mt-2" style="{{ $lawyer->profile_photo != null ? 'display:block;' : 'display:none;' }} ">
                            @if ($lawyer->profile_photo)
                                <img src="{{ asset(getUploadedImage($lawyer->profile_photo)) }}" class="img-thumbnail"
                    ...
                    <div class="">
                        ...
                        <input type="text" name="working_hours" id="working_hours" placeholder="9:00 AM - 12:00 PM"
                            class="bg-[#F9F9F9] border border-gray-300 text-gray-900 text-sm rounded-[10px] focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5 "
                            value="{{ old('working_hours', $lawyer->working_hours) }}" />
                        @error('working_hours')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="step-content hidden" data-step="1">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('frontend.emirates_id_front') }}</label>
                        <input type="file" name="emirate_id_front" id="emirates_id_frontInput" accept="image/*,application/pdf" class="bg-[#F9F9F9] border border-gray-300 text-gray-900 text-sm rounded-[10px] focus:ring-blue-500 focus:border-blue-500 block w-full">
                        @error('emirates_id_front')
                            <div class="text-sm text-red-500">{{ $message }}</div>
                        @enderror
                        <div id="emirates_id_frontPreview" class="mt-2"
                            @if ($lawyer->emirate_id_front == null) style="display:none;" @endif>

                            @if ($lawyer->emirate_id_front)
                                @php
                                    $ext = pathinfo($lawyer->emirate_id_front, PATHINFO_EXTENSION);
                                    $file = asset(getUploadedImage($lawyer->emirate_id_front));
                                @endphp
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('frontend.emirates_id_back') }}</label>
                        <input type="file" name="emirate_id_back" id="emirates_id_backInput" accept="image/*,application/pdf" class="bg-[#F9F9F9] border border-gray-300 text-gray-900 text-sm rounded-[10px] focus:ring-blue-500 focus:border-blue-500 block w-full">
                        @error('emirates_id_back')
                            <div class="text-sm text-red-500">{{ $message }}</div>
                        @enderror
                        <div id="emirates_id_backPreview" class="mt-2"
                            @if ($lawyer->emirate_id_back == null) style="display:none;" @endif>

                            @if ($lawyer->emirate_id_back)
                                @php
                                    $ext = pathinfo($lawyer->emirate_id_back, PATHINFO_EXTENSION);
                                    $file = asset(getUploadedImage($lawyer->emirate_id_back));
                                @endphp
                                @if (in_array(strtolower($ext), ['jpg', 'jpeg', 'png', 'webp', 'svg']))
                                    <img src="{{ $file }}" class="img-thumbnail"
                                        style="max-width: 200px;">
                    ...
                    <div>
                        ...
                        <input type="date"  name="emirates_id_expiry" id="emirates_id_expiry" value="{{ old('emirates_id_expiry', $lawyer->emirate_id_expiry ?? '') }}" class="bg-[#F9F9F9] border border-gray-300 text-gray-900 text-sm rounded-[10px] focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5">
                    ...
                    <div class="w-full">
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('frontend.passport') }}</label>
                        ...
                        <div id="passportPreview" class="mt-2"
                            @if ($lawyer->passport == null) style="display:none;" @endif>

                            @if ($lawyer->passport)
                                @php
                                    $ext = pathinfo($lawyer->passport, PATHINFO_EXTENSION);
                                    $file = asset(getUploadedImage($lawyer->passport));
                                @endphp
                                @if (in_array(strtolower($ext), ['jpg', 'jpeg', 'png', 'webp', 'svg']))
                                    <img src="{{ $file }}" class="img-thumbnail"
                                        style="max-width: 200px;">
                    ...
                    <div class="w-full">
                        ...
                        <input type="date" name="passport_expiry" id="passport_expiry" value="{{ old('passport_expiry', $lawyer->passport_expiry ?? '') }}"
                            class="bg-[#F9F9F9] border border-gray-300 text-gray-900 text-sm rounded-[10px] focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5">
                    ...
                    <div>
                        ...
                        <div id="residence_visaPreview" class="mt-2"
                            @if ($lawyer->residence_visa == null) style="display:none;" @endif>

                            @if ($lawyer->residence_visa)
                                @php
                                    $ext = pathinfo($lawyer->residence_visa, PATHINFO_EXTENSION);
                                    $file = asset(getUploadedImage($lawyer->residence_visa));
                                @endphp
                                @if (in_array(strtolower($ext), ['jpg', 'jpeg', 'png', 'webp', 'svg']))
                                    <img src="{{ $file }}" class="img-thumbnail"
                                        style="max-width: 200px;">
                    ...
                    <div>
                        ...
                        <input type="date" name="residence_visa_expiry" id="residence_visa_expiry" value="{{ old('residence_visa_expiry', $lawyer->residence_visa_expiry ?? '') }}"
                            class="bg-[#F9F9F9] border border-gray-300 text-gray-900 text-sm rounded-[10px] focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5">
                    ...
                    <div>
                        ...
                        @error('bar_card')
                            <div class="text-sm text-red-500">{{ $message }}</div>
                        @enderror
                        <div id="bar_cardPreview" class="mt-2" @if ($lawyer->bar_card == null) style="display:none;" @endif>

                            @if ($lawyer->bar_card)
                                @php
                                    $ext = pathinfo($lawyer->bar_card, PATHINFO_EXTENSION);
                                    $file = asset(getUploadedImage($lawyer->bar_card));
                                @endphp
                                @if (in_array(strtolower($ext), ['jpg', 'jpeg', 'png', 'webp', 'svg']))
                                    <img src="{{ $file }}" class="img-thumbnail"
                                        style="max-width: 200px;">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('frontend.ministry_of_justice_card') }}</label>
                        <input type="file"  name="practicing_lawyer_card" id="ministry_of_justice_cardInput" accept="image/*,application/pdf" class="bg-[#F9F9F9] border border-gray-300 text-gray-900 text-sm rounded-[10px] focus:ring-blue-500 focus:border-blue-500 block w-full">
                        @error('ministry_of_justice_card')
                            <div class="text-sm text-red-500">{{ $message }}</div>
                        @enderror
                        <div id="ministry_of_justice_cardPreview" class="mt-2"
                            @if ($lawyer->practicing_lawyer_card == null) style="display:none;" @endif>

                            @if ($lawyer->practicing_lawyer_card)
                                @php
                                    $ext = pathinfo($lawyer->practicing_lawyer_card, PATHINFO_EXTENSION);
                                    $file = asset(getUploadedImage($lawyer->practicing_lawyer_card));
                                @endphp
                                @if (in_array(strtolower($ext), ['jpg', 'jpeg', 'png', 'webp', 'svg']))
                                    <img src="{{ $file }}" class="img-thumbnail"
                                        style="max-width: 200px;">
                    ...
                    <div>
                        ...
                        <input type="date" name="ministry_of_justice_card_expiry" id="ministry_of_justice_card_expiry" value="{{ old('ministry_of_justice_card_expiry', $lawyer->practicing_lawyer_card_expiry ?? '') }}"
                            class="bg-[#F9F9F9] border border-gray-300 text-gray-900 text-sm rounded-[10px] focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5">
            ...
            <div class="step-content hidden" data-step="2">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="w-full">
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('frontend.password') }}</label>
                    ...
                    <div class="w-full">
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('frontend.confirm_password') }}</label>
...
@section('script')
    <script>
        ...
        let form = $("#lawyerForm").validate({
            ignore: [], 
            rules: {
                'translations[en][name]': { required: true }, 
