...
<head>
    ...
    <title>Zoom Test - Method 3 (Simplified)</title>
...
<body class="bg-gray-100">
    <div class="container mx-auto px-4 py-8">
        <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
            <h1 class="text-3xl font-bold mb-4">Method 3: Simplified Video Elements</h1>
            <p class="text-gray-600 mb-4">This uses basic HTML5 video elements with startVideo()</p>
            ...
            <div class="bg-purple-50 border border-purple-200 rounded p-4 mb-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="flex items-end">
                        <button onclick="startTest()" class="bg-purple-600 hover:bg-purple-700 text-white font-bold px-6 py-2 rounded w-full">
                            Start Test
                        </button>
                    </div>
                </div>
        ...
        <div id="videoArea" class="hidden">
            <div class="bg-white rounded-lg shadow-lg p-6">
                <div class="relative">
                    <div id="videoContainer" class="w-full h-[600px] bg-gradient-to-br from-purple-900 to-purple-800 rounded-xl relative overflow-hidden shadow-2xl flex gap-4 p-4"></div>
                    ...
                    <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-6">
                        <div class="flex items-center justify-center gap-4">
                            ...
                            <button id="toggleVideoBtn" class="bg-gray-700 hover:bg-gray-600 text-white p-4 rounded-full">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="absolute top-4 left-4 bg-black/60 backdrop-blur-sm px-4 py-2 rounded-lg">
                        <p class="text-white text-sm font-medium">Method 3 Test</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        async function startTest() {
            ...
            try {
                ...
                selfVideo.className = 'w-1/2 h-full rounded-lg border-4 border-blue-500 object-cover';
                selfVideo.autoplay = true;
                selfVideo.muted = true;
                container.appendChild(selfVideo);

                log('Starting local video with video element...');
                ...
                client.on('user-added', async (payload) => {
                    ...
                    remoteVideo.id = `remote-video-${remoteUser.userId}`;
                    remoteVideo.className = 'w-1/2 h-full rounded-lg border-4 border-green-500 object-cover';
                ...
                client.on('peer-video-state-change', async (payload) => {
                    ...
                    if (payload.action === 'Start' && remoteVideo) {
                        try {
                            await stream.startVideo({ videoElement: remoteVideo, userId: remoteUserId });
                            log(`Remote video started for user ${remoteUserId}`);
                        } catch (err) {
                            log(`ERROR starting remote video: ${err.message}`);
                        }
                    }
                });
                ...
                client.on('user-removed', async (payload) => {
                    ...
                    const remoteVideo = document.getElementById(`remote-video-${remoteUser.userId}`);
                    if (remoteVideo) {
                        remoteVideo.remove();
                    }
                });
                ...
                log('✓ Method 3 test started successfully!');
        ...
        log('Method 3 page loaded - Ready to test');
    </script>
</body>
</html>
