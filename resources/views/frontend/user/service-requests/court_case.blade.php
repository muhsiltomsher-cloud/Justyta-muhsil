...
@section('content')
    <form method="POST" action="{{ route('service.court-case-request') }}" id="courtCaseForm" enctype="multipart/form-data">
        @csrf
...
@section('ads')
    @php
        $ads = getActiveAd('court_case_submission', 'web');
    @endphp
        ...
        $(document).ready(function () {
            ...
            function loadEmiratesCaseTypes(litigationType) {
                $.ajax({
                    ...
                    type: "GET",
                    data: { litigation_type: litigationType, service: 'court-case-submission' },
