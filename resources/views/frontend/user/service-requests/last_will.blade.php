...
@section('content')
    <form method="POST" action="{{ route('service.last-will-request') }}" id="lastWillForm" enctype="multipart/form-data">
        @csrf
            ...
            <div class="lg:col-span-2 bg-white p-10 rounded-[20px] border !border-[#FFE9B1]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-6">
                    <div class="border-b pb-6 col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-3">{{ __('frontend.testament_place')  }}<span class="text-red-500">*</span></label>
                        <div class="flex items-center space-x-4">
                            <div class="flex items-center">
                                <input id="testament-company" type="radio" value="company" name="testament_place" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500" {{ (old('testament_place', 'company') == 'company') ? 'checked' : '' }} />
                                <label for="testament-company" class="ms-2 text-sm text-gray-900">{{ __('frontend.company')  }}</label>
                            </div>
                            <div class="flex items-center">
                                <input id="testament-individual" type="radio" value="individual" name="testament_place" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500"  {{ (old('testament_place') == 'individual') ? 'checked' : '' }}/>
                                <label for="testament-individual" class="ms-2 text-sm text-gray-900">{{ __('frontend.individual')  }}</label>
                            </div>
                        </div>
                        @error('testament_place')
                    ...
                    <div>
                        <label for="religion" class="block text-sm font-medium text-gray-700 mb-2">{{ __('frontend.religion') }}<span class="text-red-500">*</span></label>
                        <select id="religion" name="religion" class="bg-[#F9F9F9] border border-gray-300 text-gray-900 text-sm rounded-[10px] focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5">
                            <option value="">{{ __('frontend.choose_option') }}</option>
                            @foreach ($dropdownData['religion'] as $religion)
                                <option value="{{ $religion['id'] }}" {{ old('religion') == $religion['id'] ? 'selected' : '' }}>{{ $religion['value'] }}</option>
                            @endforeach
                        </select>
                        @error('religion')
                            <span class="text-red-500">{{ $message }}</span>
        ...
        $(document).ready(function () {
            ...
            $("#lastWillForm").validate({
                ...
                messages: {
                    ...
                    religion: "{{ __('messages.religion_required') }}",
