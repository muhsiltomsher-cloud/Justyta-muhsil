...
@section('content')
    <form method="POST" action="{{ route('service.debts-collection-request') }}" id="debtsCollectionForm" enctype="multipart/form-data">
        @csrf
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 bg-white p-10 rounded-[20px] border !border-[#FFE9B1]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-6">
                    ...
                    <div>
                        <label for="debt_type" class="block text-sm font-medium text-gray-700 mb-2">{{ __('frontend.debt_type') }}<span class="text-red-500">*</span></label>
                        <select id="debt_type" name="debt_type" class="select2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5">
                            <option value="">{{ __('frontend.choose_option') }}</option>
                            @foreach ($dropdownData['debt_type'] as $debt_type)
                                <option value="{{ $debt_type['id'] }}"  {{ (old('debt_type') == $debt_type['id']) ? 'selected' : '' }}>{{ $debt_type['value'] }}</option>
                            @endforeach
                        </select>
                        @error('debt_type')
                            <span class="text-red-500">{{ $message }}</span>
                    ...
                    <div>
                        <label for="case-stage" class="block text-sm font-medium text-gray-700 mb-2">{{ __('frontend.debt_amount') }}<span class="text-red-500">*</span></label>
                        <input type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5" placeholder="{{ __('frontend.enter_amount') }}" name="debt_amount" id="debt_amount" value="{{ old('debt_amount') }}">
                        @error('debt_amount')
                            <span class="text-red-500">{{ $message }}</span>
                        @enderror 
                    </div>
                    ...
                    <div>
                        <label for="debt_category" class="block text-sm font-medium text-gray-700 mb-2">
                            {{ __('frontend.debt_category') }}<span class="text-red-500">*</span></label>

                        <select id="debt_category" name="debt_category" class="select2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5">
                            <option value="">{{ __('frontend.choose_option') }}</option>
                            @foreach ($dropdownData['debt_category'] as $debtCategory)
                                <option value="{{ $debtCategory['id'] }}" {{ (old('debt_category') == $debtCategory['id']) ? 'selected' : '' }}>{{ $debtCategory['value'] }}</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-2 gap-x-6 gap-6">
                    ...
                    <div>
                        <label for="documents" class="block text-sm font-medium text-gray-700 mb-2">
                            {{ __('frontend.any_supporting_document') }}
        ...
        $(document).ready(function () {
            ...
            $("#debtsCollectionForm").validate({
                ...
                messages: {
                    ...
                    debt_amount: "{{ __('messages.debt_amount_required') }}",
                    emirate_id: "{{ __('messages.emirate_required') }}",
                    debt_type: "{{ __('messages.debt_type_required') }}",
                    debt_category: "{{ __('messages.debt_category_required') }}",
                    "documents[]": {
