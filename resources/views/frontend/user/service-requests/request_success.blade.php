    ...
    <div class="grid grid-cols-1 gap-6">
        <div class=" bg-white p-10 rounded-[20px] border !border-[#FFE9B1] h-[calc(100vh-150px)]">
            <div class="bg-[#FFFAF0] border !border-[#FFE9B1] rounded-xl p-8 max-w-xl m-auto w-full text-center">
                ...
                <h1 class="text-2xl font-light text-gray-800 mb-4">{{ __('frontend.request_submit_success') }}</h1>

                <div class="bg-[#FFF9F4] p-8 rounded-lg mb-6 border border-[#F5E4BA]">
                    <p class="text-gray-700 text-sm mb-3">{{ __('frontend.application_reference_number') }}</p>
                    <div
                        class=" items-center justify-center bg-[#F1E8DA] inline-flex w-auto rounded-md py-2 px-4 border border-gray-200">
                        <span class="font-bold text-lg text-[#4D1717] mr-3">#<span id="referenceText"> {{ $data['reference'] }}</span></span>
                        <button type="button" class="cursor-pointer text-gray-600 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-300 rounded-md p-1"  aria-label="{{ __('frontend.copy_ARN') }}" onclick="copyReference()">
                            <svg class="w-6 h-6 text-[#4D1717]" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <span id="copyMessage" class="relative left-0 text-sm text-green-600 font-medium hidden">{{ __('frontend.copied') }}</span>
                    </div>
                </div>

                <div class="text-gray-600 mb-6">{!! $data['message'] !!}</div>
