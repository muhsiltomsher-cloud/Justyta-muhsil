@extends('layouts.admin_default', ['title' => 'Update Pricing'])
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="breadcrumb-main">
                    <h4 class="text-capitalize breadcrumb-title">Update Pricing</h4>
                </div>
            </div>
        </div>
            ...
            <div class="col-lg-12">
                <div class="card card-default card-md mb-4">
                    <div class="card-body pb-md-30">
                        <form action="{{ route('request-pricing.update', $pricing->id) }}" method="POST" enctype="multipart/form-data"  autocomplete="off">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <!-- Law Firm Details -->
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-3 mb-3">
                                            ...
                                            <select name="litigation_place" id="litigation_place" class="form-control">
                                                ...
                                                <option value="court" {{ old('litigation_place', $pricing->litigation_place) == 'court' ? 'selected' : '' }}>
                                                    Court
                                                </option>
                                                <option value="public_prosecution" {{ old('litigation_place', $pricing->litigation_place) == 'public_prosecution' ? 'selected' : '' }}>
                                                    Public Prosecution
                                                </option>
                                        ...
                                        <div class="col-md-3 mb-3">
                                            ...
                                            <input type="number" step="0.01" name="govt_amount" id="govt_amount" value="{{ old('govt_amount', $pricing->govt_fee) }}" class="form-control" />
                                            @error('govt_amount')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="col-md-3 mb-3">
                                            ...
                                            <input type="number" readonly step="0.01" name="total_amount" id="total_amount" value="{{ old('total_amount', $pricing->total_amount) }}" class="form-control" />
                                            @error('total_amount')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
...
<script>
    $(document).ready(function () {
        ...
        function loadCaseTypesAndRequestTypes(callback = null) {
            ...
            if (litigationType && litigationPlace) {
                $.get("{{ route('filter-case-types') }}", { litigation_type: litigationType, litigation_place: litigationPlace }, function (data) {
                    ...
                    let oldCaseType = "{{ old('case_type', $pricing->case_type_id) }}";
                ...
                $.get("{{ route('filter-request-types') }}", { litigation_type: litigationType, litigation_place: litigationPlace }, function (data) {
                    ...
                    let oldRequestType = "{{ old('request_type', $pricing->request_type_id) }}";
        ...
        function loadRequestTitles(requestTypeId, callback = null) {
            if (requestTypeId) {
                $.get("{{ route('filter-request-titles') }}", { request_type_id: requestTypeId }, function (data) {
                    ...
                    let oldRequestTitle = "{{ old('request_title', $pricing->request_title_id) }}";
        ...
        @if(old('litigation_type', $pricing->litigation_type) && old('litigation_place', $pricing->litigation_place))
