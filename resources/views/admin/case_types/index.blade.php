...
@section('content')
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="breadcrumb-main">
                    <h4 class="text-capitalize breadcrumb-title">Case Types</h4>
                    <div class="breadcrumb-action justify-content-center flex-wrap">
                        @can('add_dropdown_option')
                            <div class="action-btn">
                                <button class="btn btn-primary btn-sm btn-add" id="addCaseTypeBtn">
                                    <i class="la la-plus"></i>Add Case Type
                                </button>
                            </div>
                        @endcan
                        ...
                        <div class="table4  bg-white mb-30">
                            <form class="row mb-2" id="sort_brands" action="" method="GET" autocomplete="off">
                                <div class="col-md-5 input-group  mb-1">
                                    <input type="text" class="form-control ih-small ip-gray radius-xs b-light px-15"
                                        ...
                                        placeholder="Type Case Type name..">
                                </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-basic mb-0">
                                    ...
                                    <tbody id="caseTypesTable">
                                        @can('view_dropdown_option')
                                            @if ($case_types->isNotEmpty())
                                                @foreach ($case_types as $key => $type)
                                                    <tr data-id="{{ $type->id }}">
                                                        <td class="text-center ">
                                                            {{ $key + 1 + ($case_types->currentPage() - 1) * $case_types->perPage() }}
                                                        </td>
                                                        ...
                                                        <td class="text-center">
                                                            @can('edit_dropdown_option')
                                                                <div class="table-actions">
                                                                    <a class="edit-btn" data-id="{{ $type->id }}" title="Edit Case Type" style="cursor: pointer;">
                                ...
                                <div class="aiz-pagination mt-4">
                                    @can('view_dropdown_option')
                                        {{ $case_types->appends(request()->input())->links('pagination::bootstrap-5') }}
                                    @endcan
                                </div>
                            </div>
                        </div>
    ...
    <!-- Modal -->
    <div class="modal fade" id="case_typeModal" tabindex="-1" aria-labelledby="modalLabel" aria-hidden="true" autocomplete="off">
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            let modal = new bootstrap.Modal(document.getElementById('case_typeModal'));
            const storeUrl = "{{ url('admin/case-types') }}";
            const updateUrl = "{{ url('admin/case-types') }}/"; // Append ID dynamically
            const editUrl = "{{ url('admin/case-types/edit') }}/";

            $('#addCaseTypeBtn').click(function() {
            ...
            $('.edit-btn').click(function() {
                $('.modal-title').text('Edit Case Type');
                $('#formErrors').addClass('d-none');
                const id = $(this).data('id');
                editCaseType(id);
            });

            function editCaseType(id) {
            ...
            $('#CaseTypeForm').submit(function(e) {
                ...
                let formData = $('#CaseTypeForm').serialize();
