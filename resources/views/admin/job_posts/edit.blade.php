@extends('layouts.admin_default', ['title' => 'Update Job Post'])
            ...
            <div class="col-lg-12 mx-auto">
                <div class="card card-horizontal card-default card-md mb-4">
                    <div class="card-header">
                        <h5 class="mb-0 h4">Update Job Post</h5>
                    </div>
                    <div class="card-body pb-md-30">
                        <form class="form-horizontal row" autocomplete="off" action="{{ route('job-posts.update', $jobPost->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                                ...
                                <select name="emirate" class="form-control">
                                    ...
                                    @foreach (\App\Models\Emirate::with('translations')->get() as $emirate)
                                        <option value="{{ $emirate->id }}"
                                            {{ old('emirate',$jobPost->emirate) == $emirate->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select name="type" class="form-control">
                                    ...
                                    <option value="full_time" {{ old('type', $jobPost->type) == 'full_time' ? 'selected' : '' }}>Full Time</option>
                                    <option value="part_time" {{ old('type', $jobPost->type) == 'part_time' ? 'selected' : '' }}>Part Time</option>
                                </select>
                                @error('type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <input type="text" name="deadline_date"
                                    ...
                                    value="{{ old('deadline_date', $jobPost->deadline_date ? \Carbon\Carbon::parse($jobPost->deadline_date)->format('d F Y') : '') }}" />
                                @error('deadline_date')
